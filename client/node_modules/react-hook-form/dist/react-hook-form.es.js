...
import { useState as useState$1, useRef as useRef$1, useEffect as useEffect$1, isValidElement, cloneElement, createElement, createContext, useContext } from 'react';
...
function useForm({ mode = VALIDATION_MODE.onSubmit, reValidateMode = VALIDATION_MODE.onChange, validationSchema, defaultValues = {}, submitFocusError = true, validateCriteriaMode, } = {}) {
    ...
    const executeSchemaValidation = useCallback(async (payload, shouldRender) => {
        ...
        if (isArray(payload)) {
            ...
            renderBaseOnError(fieldName, error ? { [fieldName]: error } : {}, shouldRender || previousFormIsValid !== isValidRef.current);
        }
...
const Controller = (_a) => {
    var { name, rules, as: InnerComponent, onChange, onBlur, onChangeName, onBlurName, control: { defaultValues, fields, setValue, register, unregister, errors, mode: { isOnSubmit, isOnBlur }, reValidateMode: { isReValidateOnBlur, isReValidateOnSubmit }, formState: { isSubmitted }, } } = _a, rest = __rest(_a, ["name", "rules", "as", "onChange", "onBlur", "onChangeName", "onBlurName", "control"]);
    const [value, setInputStateValue] = useState$1(isUndefined(defaultValues[name]) ? '' : defaultValues[name]);
    ...
    useEffect$1(() => () => unregister(name), []);
    const props = Object.assign(Object.assign(Object.assign(Object.assign({}, (onChange
        ? {
            [onChangeName || VALIDATION_MODE.onChange]: eventWrapper(onChange, VALIDATION_MODE.onChange),
        }
        : { onChange: handleChange })), (isOnBlur || isReValidateOnBlur
        ? onBlur
            ? {
                [onBlurName || VALIDATION_MODE.onBlur]: eventWrapper(onBlur, VALIDATION_MODE.onBlur),
            }
            : { onBlur: handleBlur }
        : {})), (isCheckboxInput ? { checked: value } : { value })), rest);
...
export { Controller, FormContext, useForm, useFormContext };
